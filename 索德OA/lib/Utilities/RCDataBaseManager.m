...
- (FMDatabaseQueue *)dbQueue {
    ...
    if (!_dbQueue) {
        ...
        NSString *filePath = [paths[0] stringByAppendingPathComponent:@"RongCloud"];
        ...
        NSString *dbPath = [filePath
            stringByAppendingPathComponent:[NSString
                                               stringWithFormat:@"RongIMDemoDB%@.db",
...
- (RCDUserInfo *)getUserByUserId:(NSString *)userId {
    ...
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:@"SELECT * FROM FRIENDSTABLE where userid = ?", userId];
        while ([rs next]) {
            model = [[RCDUserInfo alloc] init];
            model.userId = [rs stringForColumn:@"userid"];
            model.name = [rs stringForColumn:@"name"];
...
- (void)insertGroupMemberToDB:(NSMutableArray *)groupMemberList
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        [self.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
            [db executeUpdate:deleteSql];
            for (RCDUserInfo *user in groupMemberList) {
...
- (void)insertFriendListToDB:(NSMutableArray *)FriendList complete:(void (^)(BOOL result))result {
